...
if seccion == "Home":
    ...
    with col_img:
        try:
            ...
            st.warning("Archivo logo_lopez.png no hallado")
    with col_info:
...
# EJERCICIO 1
    ...
    st.markdown("Modulo para el registro de movimientos financieros en una lista dinamica.")
...
# EJERCICIO 2
    ...
    st.markdown("Formulario para registrar informacion de productos utilizando arreglos de NumPy para el procesamiento de datos.")[cite: 3]
    
    with st.container(border=True):
        col_n, col_cat = st.columns(2)
        with col_n:
            nombre_p = st.text_input("Nombre del producto")[cite: 3]
        with col_cat:
            categoria_p = st.selectbox("Categoria", ["Electronica", "Mobiliario", "Suministros", "Otros"])[cite: 3]
        
        col_pre, col_can = st.columns(2)
        with col_pre:
            precio_p = st.number_input("Precio", min_value=0.0, step=0.1)[cite: 3]
        with col_can:
            cantidad_p = st.number_input("Cantidad", min_value=0, step=1)[cite: 3]
            
        if st.button("Agregar nuevo registro", use_container_width=True):[cite: 3]
            if nombre_p:
                # Almacenamiento en arrays de NumPy
                total_p = precio_p * cantidad_p
                nuevo_registro = np.array([nombre_p, categoria_p, precio_p, cantidad_p, total_p])[cite: 3]
                st.session_state.db_inventario.append(nuevo_registro)[cite: 3]
    ...
    if st.session_state.db_inventario:
        ...
        df_inventario = pd.DataFrame(st.session_state.db_inventario, columns=columnas)[cite: 3]
        ...
        st.dataframe(df_inventario, use_container_width=True, hide_index=True)[cite: 3]
